...
/**
 * Compact topbar control: Open in ▾
 * Shows only targets the host can actually launch.
 */
export function OpenInControls({ projectPath, onError }: Props) {
  const [open, setOpen] = useState(false);
  const [busy, setBusy] = useState(false);
  const [options, setOptions] = useState<OpenInOption[]>([]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    void fetchOpenInOptions(path)
      .then((res) => {
        if (cancelled) return;
        setOptions(res.options.filter((o) => o.available));
      })
      .catch((e) => {
        if (cancelled) return;
        setOptions([]);
        onErrorRef.current?.(
          safeErrorMessage(e, "Could not load available applications."),
        );
      })
      .finally(() => {
  ...
  const run = async (target: OpenInTarget) => {
    setBusy(true);
    try {
      ...
      onErrorRef.current?.(
        safeErrorMessage(e, "Could not open the selected application."),
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="open-in" ref={rootRef}>
      <button
        type="button"
        className={`topbar__chip open-in__btn${open ? " is-open" : ""}`}
      ...
      {open ? (
        <div className="open-in__menu" role="menu">
          {loading ? (
            <div className="open-in__meta">Checking apps…</div>
          ) : options.length === 0 ? (
            <div className="open-in__meta">No apps available</div>
          ) : (
            options.map((opt) => (
              <button
                key={opt.id}
              ...
              >
                <TargetIcon id={opt.id} label={opt.label} />
                <span>{opt.label}</span>
              </button>
            ))
          )}
...
function TargetIcon({ id, label }: { id: OpenInTarget; label: string }) {
  switch (id) {
    case "explorer":
      // Windows File Explorer uses the Windows brand mark; other OSes keep a folder glyph.
      return label === "File Explorer" ? (
        <WindowsBrandIcon />
      ) : (
        <FolderOpenIcon />
      );
    case "terminal":
      // Windows Terminal brand on Windows; generic terminal glyph elsewhere.
      return typeof navigator !== "undefined" &&
        /windows/i.test(navigator.userAgent) ? (
        <WindowsTerminalBrandIcon />
      ) : (
        <TerminalGlyphIcon />
      );
    case "gitBash":
      return <GitBrandIcon />;
    case "wsl":
      return <LinuxBrandIcon />;
  }
}

/** Generic open-folder glyph for the trigger button (not a product brand). */
function FolderOpenIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden>
      <path
        d="M3.5 7.5A2 2 0 0 1 5.5 5.5h4l1.5 1.5H18.5a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-13a2 2 0 0 1-2-2v-9.5Z"
        stroke="currentColor"
        strokeWidth="1.7"
...
/** Microsoft Windows brand mark (File Explorer on Windows). */
function WindowsBrandIcon() {
  return (
    <svg
      className="open-in__brand-icon"
      width="14"
      height="14"
      viewBox="0 0 24 24"
      fill="currentColor"
...
/** Windows Terminal brand mark. */
function WindowsTerminalBrandIcon() {
  return (
    <svg
      className="open-in__brand-icon"
      width="14"
...
/** Generic terminal glyph when the host is not Windows Terminal. */
function TerminalGlyphIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden>
      ...
      />
      <path
        d="m7.5 9.5 3 2.5-3 2.5M12.5 14.5h4"
        stroke="currentColor"
        strokeWidth="1.7"
        strokeLinecap="round"
        strokeLinejoin="round"
...
/** Git brand mark (Git for Windows / Git Bash). */
...
/** Linux / WSL brand mark (Tux). */
function LinuxBrandIcon() {
  return (
    <svg
      className="open-in__brand-icon"
      width="14"
